...
// This code covers the attenuation part
// You can calibrate the solenoid remotely
...
void setup() {
  ...
  // Start with activated
  digitalWrite(pin_Relay_5, HIGH);

  radio.read( &mystruct_1, sizeof(mystruct_1) );
  
  Serial.println(F("Bring it to -90° ... "));

  while (counter_stable < 100) {
    if ( radio.available()){
      radio.read( &mystruct_1, sizeof(mystruct_1) );
      delay(5);
    if (mystruct_1.z_ddot > -0.05 && mystruct_1.z_ddot < 0.05) {
  ...
  Serial.println(F(" ---> -90° are confirmed"));
...
void loop() {
  ...
  while (radio.available()) {
    ...
    if (mystruct_1.y_dot != 0) {
      ...
      // after 1/4 of time period in contracted mode minus dead time of releasing (0ms works better than 100ms)
      if ((millis() > (switch_time_s_n + (switch_time_s_n - switch_time_s_n1) / 2 - 0)) && bit_switch == 1) {
        digitalWrite(pin_Relay_5, LOW);
        bit_switch = 0;
      }

      Serial.print(time_n_ms);  Serial.print(", ");
      Serial.print(vel_filtered_n); Serial.println(", ");
