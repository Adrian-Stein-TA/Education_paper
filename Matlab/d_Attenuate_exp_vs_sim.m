%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Comparing experiments to simulation for activated solenoids (attenuate) %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
alpha_rad_sim = angle_init_deg/360*2*pi; % because you hold the pendulum at -90° initial condition
vel_radps_sim = data_mat(1,2)/360*2*pi; % angular velocity initial condition comes from the measured data

% defining offsets as global variables
% assuming angle_off = 0°
global angle_off;
angle_off = 0/360*2*pi; % in rad

bit_offset = 0; % choosing no offset in angle
...
% assuming angle_off = 10°
angle_off = 10/360*2*pi; % in rad

bit_offset = 1; % choosing an offset in angle
...
grid on;
box on;
legend('real (with solenoid)','simulated (angle_{off} = 0 deg)', 'simulated (angle_{off} = 10 deg)')
...
for i=1:n
    if start == 1
        ...
        L_var = L_activated_sim; % start in with an activated solenoid mode
    ...
    elseif abs(xx(end,1)) < abs(xx(end,2)) % we are at phi = 0 (lowest point) (assumption: phi is way smaller than phi_dot)
        ...
        y0(2) = (L_activated_sim/L_deactivated_sim)^2*xx(end,2); % as an approximate for small perturbations around phi = 0; % phi in rad/s
...
% function for 10° offset
function [value,isterminal,direction] = yzero_offset(~,y) % Locate the time when the angle and angular velocity is zero
global angle_off;
